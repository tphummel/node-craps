function minPassLineOnly (opts) {
  ...
  if (process.env.DEBUG) console.log(`[MPLO  decision] make a new pass line bet?: ${hand.isComeOut} && ${!bets?.pass?.line} => ${(hand.isComeOut) && (!bets?.pass?.line)}`)
...
function minPassLineMaxOdds (opts) {
  ...
  if (process.env.DEBUG) console.log(`[MPLMO decision] make a new pass odds bet?: ${!hand.isComeOut} && ${!bets?.pass?.odds} => ${(!hand.isComeOut) && (!bets?.pass?.odds)}`)
